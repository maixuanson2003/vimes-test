...
export class AddReceiptUserRoles1788100000000 implements MigrationInterface {
  async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TYPE user_role ADD VALUE IF NOT EXISTS 'PREPARER'`,
    );
    await queryRunner.query(
    ...
    `);
    await queryRunner.query(
      `CREATE INDEX idx_phieu_nhap_nguoi_lap ON phieu_nhap_kho(nguoi_lap_id)`,
    );
    await queryRunner.query(
      `CREATE INDEX idx_phieu_nhap_thu_kho ON phieu_nhap_kho(thu_kho_id)`,
    );
    await queryRunner.query(
      `CREATE INDEX idx_phieu_nhap_ke_toan_truong ON phieu_nhap_kho(ke_toan_truong_id)`,
    );
  }

  async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP INDEX IF EXISTS idx_phieu_nhap_ke_toan_truong`,
    );
