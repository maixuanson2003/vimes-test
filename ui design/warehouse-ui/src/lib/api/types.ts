...
export type GoodsReceiptItem = Entity & {
  receiptId: number;
  productId: number;
...
export type GoodsReceipt = Entity & {
  ...
  chiefAccountant?: User | null;
  createdAt?: string;
  items?: GoodsReceiptItem[];
};
export type GoodsReceiptAttachment = Entity & {
  receiptId: number;
  originalName: string;
  mimeType: string;
  size: number;
...
export type GoodsIssueItem = Entity & {
  issueId: number;
  productId: number;
  documentQuantity: number;
  quantity: number;
